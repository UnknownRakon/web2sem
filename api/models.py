...
class Company(models.Model):
    ...
    company_name = models.CharField(
        verbose_name='Имя компании', max_length=255)
...
class Vacancy(models.Model):
    ...
    responses = models.ManyToManyField(
        Candidate, verbose_name='Отклики', blank=True)
    STATUS_CHOICES = (('MODERATING', 'Moderating'),
                      ('ACTIVE', 'Active'), ('ARCHIVED', 'Archived'))
    status = models.CharField(
        verbose_name='Статус', max_length=255, choices=STATUS_CHOICES, default='Moderating')
    history = HistoricalRecords()
...
class Interview(models.Model):
    ...
    candidate = models.ForeignKey(
        Candidate, verbose_name='Кандидат', on_delete=models.CASCADE)
    planned_datetime = models.DateTimeField(
        verbose_name='Дата и время проведения', auto_now=False, auto_now_add=False)
    STATUS_CHOICES = (('PLANNED', 'Planned'), ('IN_PROGRESS',
                      'In_progress'), ('FINISHED', 'Finished'))
    status = models.CharField(
        verbose_name='Статус', max_length=255, choices=STATUS_CHOICES, default='Planned')
    link = models.CharField(
        verbose_name='Ссылка на собеседование', max_length=255, blank=True)
    history = HistoricalRecords()
